...
class AdaptiveTemplateEngine:
    ...
    async def _apply_semantic_refinement(self, content: str, design_tokens: Dict, title: str = "", topic: str = "", history: List[Dict] = None):
        ...
        try:
            ...
            # 构建本轮请求
            user_prompt = prompt_loader.get_adaptive("adaptive_refiner", "user_prompt").format(
                title=title, topic=topic, content=content[:2000]
            )
    ...
    async def _apply_professional_annotation(self, content: str, design_tokens: Dict, title: str = "", topic: str = "", history: List[Dict] = None):
        ...
        try:
            ...
            user_prompt = prompt_loader.get_adaptive("professional_annotation", "user_prompt").format(
                title=title, topic=topic, content=content[:2000]
            )
            messages = history + [{"role": "user", "content": user_prompt}]
