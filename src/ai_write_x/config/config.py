...
class Config:
    ...
    def __init__(self):
        ...
        self.default_config = {
            ...
            "api": {
                ...
                "deleted_providers": [],  # 用户主动删除的提供商黑名单
            },
            "img_api": {
                "api_type": "picsum",
            ...
            "dimensional_creative": {
                "enabled": True,
                "creative_intensity": 0.7,  # 降低默认强度
                "preserve_core_info": True,
                "allow_experimental": False,
                "auto_dimension_selection": True,
                "selected_dimensions": [],
                "priority_categories": ["audience", "format", "emotion", "theme"],  # 重新排序
                "max_dimensions": 3,  # 降低最大维度数
                "compatibility_threshold": 0.7,  # 提高兼容性要求
    ...
    def api_key_name(self):
        with self._lock:
            ...
            # 自定义API可能没有key字段，使用默认值
            return provider_config.get("key", "OPENAI_API_KEY")
...
"""
    ...
    def validate_config(self):
        ...
        try:
            ...
            if self.img_api_type != "picsum":
                ...
                # ComfyUI不需要api_key，只需要api_base
                if self.img_api_type == "comfyui":
                    api_base = img_api_config.get("api_base", "")
                    if not api_base:
                        self.error_message = "未配置ComfyUI的API地址，请打开配置填写"
                        return False
                else:
                    ...
                    if self.img_api_type == "custom" and isinstance(img_api_config, list):
                        ...
                        if 0 <= custom_index < len(img_api_config):
                            ...
                            self.error_message = "未配置自定义图片API，或选择的索引无效"
                            return False

                    if not isinstance(img_api_config, dict):
                        self.error_message = f"图片生成配置格式错误: {self.img_api_type} 应为字典"
                        return False
    ...
    def merge_with_user_config(self, user_config: dict) -> dict:
        ...
        # ⚡ 持久化删除：用户主动删除的提供商不再被 default_config 还原
        ...
        if deleted_api_providers and "api" in merged_config:
            ...
            # 保留黑名单到合并结果中，确保下次保存时仍然存在
            merged_config["api"]["deleted_providers"] = deleted_api_providers
